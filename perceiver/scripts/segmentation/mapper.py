...
class SegmentationMapperCLI(CLI):
	def add_arguments_to_parser(self, parser: LightningArgumentParser) -> None:
		...
		parser.set_defaults(
			...
			# 128x128x1 config:
			{
				"model.num_latents": ((IMAGE_SIZE_XY//8) * (IMAGE_SIZE_XY//8) * 1), # 1024
				"model.num_latent_channels": ((IMAGE_SIZE_XY) // 2), # 64
				
				"model.encoder.num_frequency_bands": (NUM_CLASSES * 1), # 16
				"model.encoder.num_cross_attention_layers": IMAGE_SIZE_XY//16,
				"model.encoder.num_cross_attention_heads": IMAGE_SIZE_XY//16,
				"model.encoder.num_cross_attention_qk_channels": IMAGE_SIZE_XY//8,
				"model.encoder.num_cross_attention_v_channels":IMAGE_SIZE_XY//8,
				"model.encoder.num_self_attention_blocks": IMAGE_SIZE_XY//16,
				"model.encoder.num_self_attention_layers_per_block": IMAGE_SIZE_XY//16,
				"model.encoder.num_self_attention_heads": IMAGE_SIZE_XY//16,
				"model.encoder.num_self_attention_qk_channels": IMAGE_SIZE_XY//8,
				"model.encoder.num_self_attention_v_channels":IMAGE_SIZE_XY//8,
				"model.encoder.dropout": 0.3,

				"model.decoder.num_cross_attention_heads": IMAGE_SIZE_XY//16,
				"model.decoder.num_cross_attention_qk_channels": IMAGE_SIZE_XY//8,
				"model.decoder.num_cross_attention_v_channels": IMAGE_SIZE_XY//8,
				"model.decoder.cross_attention_residual": True,
				"model.decoder.dropout": 0.3,
			}
		)
